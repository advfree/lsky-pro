            ...
            <form action="{{ route('admin.group.update', ['id' => $group->id]) }}" method="POST">
                <div class="overflow-hidden rounded-md rounded-l-none shadow-custom">
                        ...
                        <div data-tab="basic" class="grid grid-cols-6 gap-6">
                            ...
                            <div class="col-span-6">
                                <x-fieldset title="生成分享优化图" faq="开启后保留上传原图，另存一份压缩后的分享图；复制链接、Markdown、HTML 默认使用优化图。">
                                    <x-switch id="configs[is_enable_optimized_share]" name="configs[is_enable_optimized_share]" value="1" :checked="(bool)$group->configs->get('is_enable_optimized_share')"></x-switch>
                                </x-fieldset>
                            </div>
                            ...
                            <div class="col-span-6 sm:col-span-3">
                                ...
                                <x-select id="configs[optimized_image_format]" name="configs[optimized_image_format]" autocomplete="optimized_image_format">
                                    @foreach(['webp', 'jpg', 'jpeg', 'png'] as $extension)
                                        <option value="{{ $extension }}" @selected($group->configs->get('optimized_image_format', 'webp') === $extension)>{{ strtoupper($extension) }}</option>
                                    @endforeach
                                </x-select>
                            </div>
                            ...
                            <div class="col-span-6 sm:col-span-3">
                                ...
                                <x-input type="number" name="configs[optimized_image_quality]" id="optimized_image_quality" autocomplete="optimized_image_quality" placeholder="请输入分享优化图质量" value="{{ $group->configs->get('optimized_image_quality', 75) }}" />
                                <p class="mt-1 text-xs text-gray-500">范围 1-100，数字越大质量越好、文件越大。</p>
                            </div>

                            <div class="col-span-6 sm:col-span-3">
                                <label for="optimized_image_max_width" class="block text-sm font-medium text-gray-700">分享优化图最长边</label>
                                <x-input type="number" name="configs[optimized_image_max_width]" id="optimized_image_max_width" autocomplete="optimized_image_max_width" placeholder="0 表示不缩放" value="{{ $group->configs->get('optimized_image_max_width', 2560) }}" />
                            </div>
                    ...
                    <div class="px-4 py-3 bg-gray-50 text-right sm:px-6">
                        <button type="button" onclick="history.go(-1)" class="inline-flex justify-center py-2 px-4 text-sm font-medium rounded-md text-gray-600 bg-black/10 hover:bg-black/20 hover:text-gray-700">取消</button>
                        <button type="submit" class="inline-flex justify-center py-2 px-4 text-sm font-medium rounded-md text-white bg-blue-500 hover:bg-blue-600">确认保存</button>
                    </div>
                </div>
            </form>
        </div>
