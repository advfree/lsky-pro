...
class GroupRequest extends FormRequest
{
    ...
    public function rules()
    {
        ...
        return [
            ...
            'configs.is_enable_optimized_share' => 'boolean',
            'configs.optimized_image_format' => 'required|in:webp,jpg,jpeg,png',
            'configs.optimized_image_quality' => 'required|integer|min:1|max:100',
            'configs.optimized_image_max_width' => 'required|integer|min:0|max:12000',
            ...
            'configs.watermark_configs.drivers.font.font' => [
                ...
                function ($attribute, $value, $fail) {
                    if (! $this->input('configs.is_enable_watermark') || $this->input('configs.watermark_configs.driver') !== 'font' || ! $value) {
                        return;
                    }
            ...
            'configs.watermark_configs.drivers.image.image' => [
                ...
                function ($attribute, $value, $fail) {
                    if (! $this->input('configs.is_enable_watermark') || $this->input('configs.watermark_configs.driver') !== 'image' || ! $value) {
                        return;
                    }
                    if (! file_exists(storage_path('app/public/'.$value))) {
                        $fail('图片文件不存在');
    ...
    public function attributes()
    {
        return [
            ...
            'configs.is_enable_optimized_share' => '是否启用分享优化图',
            'configs.optimized_image_format' => '分享优化图格式',
            'configs.optimized_image_quality' => '分享优化图质量',
            'configs.optimized_image_max_width' => '分享优化图最长边',
